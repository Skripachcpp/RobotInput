...
namespace WorkingTools.Extensions
{
    ...
    public static class SerializeExtension
    {
        ...
        public static MemoryStream ToXml<TObj>(this TObj obj, Encoding encoding = null, bool indent = true, bool omitXmlDeclaration = false) //where TObj : class
        {
            ...
            using (var xmlWriter = XmlWriter.Create(stream, xmlWriterSettings))
            {
                ...
                CacheFactory.XmlSerializer(type).Serialize(xmlWriter, obj, xmlSerializerNamespaces);
                xmlWriter.Close();
            }
           ...
           //where TObj : class
        {
            switch (serializeTo)
            {
                case SerializeTo.MemoryStream:
                    return ToXml(obj, encoding, indent ?? true, omitXmlDeclaration ?? false);
                case SerializeTo.String:
                    using (var memoryStream = ToXml(obj, encoding, indent ?? true, omitXmlDeclaration ?? true))
        ...
        { return CacheFactory.XmlSerializer(type).Deserialize(source); }
