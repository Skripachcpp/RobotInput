...
namespace WorkingTools.Factories
{
    public static class FtpWebRequestFacroty
    {
        ...
        private static string ToUrl(string ftpServerName, string ftpPath)
        {
            ...
            if (string.IsNullOrWhiteSpace(ftpPath)) throw new ArgumentOutOfRangeException("ftpPath", "путь до файла на на ftp отсутствует или является пустым");
        ...
        {
            if (ftpServerConnection == null) throw new ArgumentNullException("ftpServerConnection", "отсутствуют параметры подключения к ftp");
            if (string.IsNullOrWhiteSpace(ftpServerConnection.Server)) throw new ArgumentOutOfRangeException("ftpServerConnection", "в параметрах подключения к ftp серверу отсутствует адрес сервера");
        ...
        public static FtpWebRequest Create(string ftpPath, FtpRequestType ftpRequestMethod,
            ...
            IWebProxy proxy)
        {
            if (ftpServerName == null) throw new ArgumentNullException("ftpServerName");
            if (string.IsNullOrWhiteSpace(ftpServerName)) throw new ArgumentOutOfRangeException("ftpServerName", "в параметрах подключения к ftp серверу отсутствует адрес сервера");
