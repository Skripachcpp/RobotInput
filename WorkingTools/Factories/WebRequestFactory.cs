...
namespace WorkingTools.Factories
{
    public static class WebRequestFactory
    {
        ...
        {
            if (string.IsNullOrEmpty(url)) throw new ArgumentNullException("url", "не указан адрес запроса");
            if (string.IsNullOrEmpty(method)) throw new ArgumentNullException("method", "не указан метод (тип) запроса");
